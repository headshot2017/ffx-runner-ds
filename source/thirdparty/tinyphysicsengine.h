...
typedef TPE_Vec3 (*TPE_ClosestPointFunction)(TPE_Vec3, TPE_Unit, void*);
  ...
  TPE_ClosestPointFunction environmentFunction)
{
  for (uint16_t i = 0; i < body->connectionCount; ++i)
  {
    ...
    if (environmentFunction != 0 && TPE_LENGTH(TPE_vec3Minus(j1->position,
      environmentFunction(j1->position,TPE_JOINT_SIZE(*j1), _TPE_userData)))
      < TPE_JOINT_SIZE(*j1))
    ...
    if (environmentFunction != 0 && TPE_LENGTH(TPE_vec3Minus(j2->position,
      environmentFunction(j2->position,TPE_JOINT_SIZE(*j2), _TPE_userData)))
      < TPE_JOINT_SIZE(*j2))
  ...
  TPE_ClosestPointFunction env)
{
  ...
  if (d < 0) // collision?
  {
    if (_TPE_collisionCallback != 0 && !_TPE_collisionCallback(
        ...
        TPE_vec3Plus(j1->position,dir), _TPE_userData))
      return 0;
...
{
  TPE_Vec3 toJoint =
    TPE_vec3Minus(joint->position,env(joint->position,TPE_JOINT_SIZE(*joint), _TPE_userData));
  ...
  if (len <= TPE_JOINT_SIZE(*joint))
  {
    if (_TPE_collisionCallback != 0)
      if (!_TPE_collisionCallback(_TPE_body1Index,
        ...
        TPE_vec3Minus(joint->position,toJoint), _TPE_userData))
        return 0;
    ...
    if (len > 0)
    {
      ...
      for (int i = 0; i < TPE_COLLISION_RESOLUTION_ITERATIONS; ++i)
      {
        ...
        toJoint = TPE_vec3Minus(joint->position,env(joint->position,
          TPE_JOINT_SIZE(*joint), _TPE_userData));
    ...
    if (!success)
    {
      ...
      for (int i = 0; i < TPE_COLLISION_RESOLUTION_ITERATIONS; ++i)
      {
        ...
        toJoint = TPE_vec3Minus(joint->position,
          env(joint->position,TPE_JOINT_SIZE(*joint), _TPE_userData));
  ...
  TPE_ClosestPointFunction env)
{
  for (uint16_t i = 0; i < body->jointCount; ++i)
  {
    ...
    if (TPE_DISTANCE(joint->position,env(joint->position,size, _TPE_userData)) <= size)
      return 1;
  }
  ...
  TPE_ClosestPointFunction env)
{
  ...
  if (TPE_DISTANCE(c,env(c,d, _TPE_userData)) > d)
    return 0;
  ...
  if (world->environmentFunction != 0)
  {
    ...
    for (uint8_t j = 0; j < envGridRes; ++j)
    {
      ...
      for (uint8_t k = 0; k < envGridRes; ++k)
      {
        ...
        {
          TPE_Vec3 r = world->environmentFunction(testPoint,envGridSize, _TPE_userData);
...
{
  TPE_Vec3 p = rayPos;
  TPE_Vec3 p2 = environment(rayPos,rayMarchMaxStep, _TPE_userData);
  TPE_Unit totalD = 0;
  ...
  if (p2.x != p.x || p2.y != p.y || p2.z != p.z)
  {
    ...
    for (uint32_t i = 0; i < maxSteps; ++i)
    {
      ...
      TPE_Vec3 pTest = environment(p2,rayMarchMaxStep, _TPE_userData);
  ...
  else if (insideStepSize != 0)
  {
    ...
    for (uint32_t i = 0; i < maxSteps; ++i)
    {
      ...
      TPE_Vec3 pTest = environment(p2,16, _TPE_userData);
  ...
  if (found)
  {
    ...
    for (uint8_t i = 0; i < 128; ++i) // upper limit just in case
    {
      ...
      TPE_Vec3 pTest = environment(middle,16, _TPE_userData); // 16: just a small number
  ...
  for (uint16_t z = 0; z < gridResolution; ++z)
  {
    ...
    for (uint16_t y = 0; y < gridResolution; ++y)
    {
      ...
      for (uint16_t x = 0; x < gridResolution; ++x)
      {
        ...
        TPE_Vec3 p2 = f(p,TPE_INFINITY, _TPE_userData);
        ...
        if (p.x != p2.x || p.y != p2.y || p.z != p2.z) // only test outside
        {
          ...
          for (uint8_t i = 0; i < 3; ++i)
          {
            ...
            TPE_Vec3 p4 = f(p3,TPE_INFINITY, _TPE_userData);
          ...
          for (uint8_t zz = 0; zz < 2; ++zz)
          {
            ...
            for (uint8_t yy = 0; yy < 2; ++yy)
            {
              ...
              for (uint8_t zz = 0; zz < 2; ++zz)
              {
                if (TPE_DISTANCE(p,f(p3,TPE_INFINITY, _TPE_userData)) + allowedError < d)
                {
